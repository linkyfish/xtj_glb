...
class MoneyTransModelBak extends \App\Model
{
...
//	}
    ...
    public function stat_sum_new_usr($start, $end, $usrid)
    {
        ...
        $list = $this->UsrMoney->select(['ymd' => $ymd, 'usrid' => $usrid]);
        $data = [
...
public function stat_sum_new($start, $end, $agent_id)
    {
        $ymd = $this->ymd_array($start, $end);
        $ymd_kv = $this->ymd_array($start, $end, 'agent_');
        $agent_kv = $this->Kv->select($ymd_kv);
        $agent_son = [];
        foreach ($agent_kv as $k => $v) {
            $son = [];
            $this->agent_child_list($agent_id, $this->make_son($v), $son);
        ...
        $data = [
            ...
            'Sc_Ad' => 0, 'Cash_Ad' => 0, 'Cash_C_Ad' => 0, 'Disable_Ad' => 0, 'Sms_Ad' => 0, 'Agent_Ad' => 0];
        foreach ($list as $row) {
            if (in_array($row['agent_id'], $agent_son[$row['ymd']], 1)) {
                $data['Game_Bet'] = bcadd($data['Game_Bet'], $row['Game_Bet']);
                $data['Poker_Bet'] = bcadd($data['Poker_Bet'], $row['Poker_Bet']);
                $data['Game_Win'] = bcadd($data['Game_Win'], $row['Game_Win']);
                $data['Poker_Win'] = bcadd($data['Poker_Win'], $row['Poker_Win']);
                ...
                //xn_log($row['ymd'].' '.$row['usrid'].' '.$agent_id.' '.$row['agent_id'],'stat_sum_new');
            }
        }
    ...
    public function stat_sum_agent($start, $end, $agent_id)
    {
        ...
        $list = $this->UserMoney->select(['ymd' => $ymd,'agent_id'=>$agent_id]);
        $data = [
            'Game_Bet' => 0, 'Poker_Bet' => 0, 'Game_Win' => 0,
            'Poker_Win' => 0, 'Poker_Pump' => 0, 'Reg_Ad' => 0, 'Active_Ad' => 0,
            'Apply_Ad' => 0, 'Plat_Ad' => 0, 'Pay_Ad' => 0, 'Sg_Ad' => 0, 'Sg_Sub' => 0,
...
//				}
    ...
    public function task_useragent_sum($start)
    {
        ...
        $list = $this->select($cond, [], 'usrid,style,diamond,times,BeginBlance,EndBlance,gameID,action_id');
        $userlist = arrlist_group($list, 'usrid');
        $users = $this->Usr->select([], [], 'usrid,agent_id,parent_id');
        $users = arrlist_key_values($users, 'usrid', 'parent_id');
        ...
        $dayNewUserListGroup = arrlist_group($dayNewUserList,'parent_id');
        foreach ($dayNewUserListGroup as $parent_playerid =>$row)
        {
            if (empty($usrmoney[$parent_playerid])) {
                $user['ymd'] = $ymd;
                $user['usrid'] = $usrid;
                $user['agent_id'] = $users[$usrid];
                $user['update_at'] = $user['created_at'] = time();
                $user['Reg_User'] = count($row);;
                $this->UsrAgentMoney->insert($user);
                $hasRegArr[]= $user;
            }
        }
        return $dayNewUserListGroup;
    }
    ...
    public function  task_update_usr_monney_total($start)
    {
      ...
      foreach ($usrmoneyListGroup as $agent_id => $row)
      {
        ...
        foreach ($row as $item)
        {
            ...
            {
                $Reg_User+=1;
                if($item['Game_Bet']+$item['Poker_Bet']>=200000)
                {
                    $Reg_Reward+=30000;
                }
            }
            $subUpdate['created_at'] = time();
            $this->UsrAgentMoney->update(['usrid'=>$item['usrid'],'ymd'=>$ymd],$subUpdate);
        }

          $update['Reg_Reward'] = $Reg_Reward;
    ...
    public function task_usr_agent_money_to_account($start)
    {
        ...
        {
            $update['Total_Reward+'] = $item['Reg_Reward']+$item['Log_Reward']+$item['Self_Log_Reward'];;
            $update['Cur_Reward+'] = $item['Reg_Reward']+$item['Log_Reward']+$item['Self_Log_Reward'];
            ...
            $this->User->update(['username'=>$item['usrid']],$update);
