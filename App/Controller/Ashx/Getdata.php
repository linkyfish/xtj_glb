...
Class Getdata extends GameController
{
    ...
    public function adminReport_GetWinMaxPlayer()
    {
        ...
        $data = $this->MoneyTrans->PlayerMaxWin($cond, $player);
    ...
    public function adminReport_report_onlinePlayer()
    {
        ...
        $eDate = $this->request->param('eDate', '');
        $data = ['r1' => [], 'r2' => [], 'r3' => [], 'r4' => []];
        $dates = $this->MoneyTrans->getDatesBetweenTwoDays($sDate, $eDate);
        $r1 = $this->Usr->RegDate($sDate, $eDate);
        $r2 = $this->PlayerOline->LoginDate($dates);
        $r3 = $this->MoneyTrans->ChipsDate($sDate, $eDate);
...
//        $sorttype = $this->request->param('sorttype', 3);
        ...
        foreach ($list['results'] as $k => $v) {
            $_list[$k] = $this->Account->format($v, $time, $this->roleInfo, $this->agent_id_name);
    ...
    public function GetPlatFormReport_plat_common_5()
    {
        ...
        $this->PostTask('moneytrans_minute',[]);
        $this->response('0000', $data);
    }
    ...
    private function rankData($dateType = "day", $isQipai = false)
    {
        ...
        if ($dateType == 'day') {
            $userTotalInfo = $this->MoneyTrans->stat_sum_new($dateDayStart,$dateDayEnd,$this->default_user['id']);
        } else if ($dateType == 'month') {
            $userTotalInfo = $this->MoneyTrans->stat_sum_new($BeginDate,$dateMonthEnd,$this->default_user['id']);
        }
