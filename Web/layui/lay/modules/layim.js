...
layui.define(['layer', 'laytpl', 'upload'], function (exports) {
    ...
    function handleFiles(files,event) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            file_type = file_type.substring(0,file_type.indexOf("/")).toLowerCase();
            if (file_type !== "png" && file_type !== "jpg"&& file_type !== "image") {
                layer.msg('仅限PNG、JPG图片上传',{icon:2,anim:6})
                return;
            }
            ...
            if(size>5000) {
                layer.msg('图片不能大于5M',{icon:2,anim:6})
                return;
            }
            ...
            $.ajax({
                ...
                , success: function (res) {
                    if(res.success){
                        layer.confirm('<img src="'+res.data.src+'" style="max-width:220px;max-height:220px;margin:auto">', {title:'图片发送确认'}, function(index){
    ...
    function initDrag(that, event) {
        ...
        dropzone.addEventListener('drop', function (e) {
            ...
            e.stopPropagation()
            handleFiles(e.dataTransfer.files, event)
        ...
        that.find('.editor textarea')[0].addEventListener('paste', function (e) {
    ...
    var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
        , '<div layui-unselect  class="layim-chat-title">'
        ...
        , '{{# }; }}'
        , '</div>'
        , '<div class="layim-chat-textarea editor" contenteditable="true"><textarea type="file" value="" name="file" accept="image/*"  multiple="multiple" ></textarea></div>'
    ...
    //聊天内容列表模版
    var elemChatMain = ['<li class="{{ d.mine ? "layim-chat-mine" : "" }} cid_{{ d.cid }}" {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
        , '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>'
        , '{{# if(d.mine){ }}'
        , '{{# if(!d.isback && d.cid){ }}<i data-cid="{{d.cid}}" data-id="{{d.id}}" data-type="{{d.type}}" data-username="{{d.username}}" style="color:#01AAED" layim-event="CallBackChat">撤回</i> {{# } }}<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}'
        , '{{# } else { }}'
        , '{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i> <i class="ChatIsRead"  {{# if(d.is_read){ }}style="color:#01AAED"{{# } }}  {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}  >{{ d.read ? "已读" : "未读" }}</i>'
        , '{{# } }}'
        , '</cite></div>'
        , '<div class="layim-chat-text {{ d.isback ? "back" :"" }}">{{# if(d.isback){ }} <i>内容已撤回</i> {{# }else{ }}{{ layui.data.content(d.content||"&nbsp") }}{{# } }}</div>'
        , '</li>'].join('');
    ...
    layui.data.content = function (content) {
        ...
        content = (content || '').replace(/&(?!#?[a-zA-Z0-9]+;)/g, '&amp;')
            ...
            .replace(/image\([\s\S]+?\)\[[\s\S]*?\]\~[\s\S]*?\~/g, function (i) {
                var a = (i.match(/image\(([\s\S]+?)\)\[/) || [])[1];
                var e = (i.match(/\)\[([\s\S]*?)\]/) || [])[1];
                var g = (i.match(/\]\~([\s\S]*?)\~/) || [])[1];
                //return '<a class="layui-layim-link" target="_blank" href="' + e + '"><h2>' + g + '</h2><br><img class="layui-layim-photos" src="' + a + '"><br>点击前往</a>';
                var str = '';
                if(e){
                    str+='<a class="layui-layim-link" target="_blank" href="' + e + '">'
                }
                if(g||e){
                    str='<h2>' + (g||e) + '</h2><br>';
                }
                if(a){
                    str+='<img class="layui-layim-photos" src="' + a + '">';
                }
                if(e){
                    str+='<br>点击前往</a>'
                }
    ...
    //发送消息
    var sendMessage = LAYIM.prototype.sendMessage =  function () {
        var data = {
    ...
    //在焦点处插入内容
    var focusInsert =LAYIM.prototype.focusInsert = function (obj, str) {
    ...
    var anim = 'layui-anim-upbit', events = {
        ...
        , CallBackChat: function (jothis) {
            var thatChat = thisChat();
            var cid = jothis.data('cid');
            var id = thatChat.data.id;
            var type = jothis.data('type')||"friend";
            var username = jothis.data('username');
            //changeChat(othis);
            //console.log(type,cid,id);
            var jload = layer.load(0);
            $.post(cache.base['callbackchat'],{cid:cid,username:username},function (g) {
                var local = layui.data('layim')[cache.mine.id] || {};
                local.chatlog = local.chatlog || {};
                local.history = local.history || {};
                var chatlog = local.chatlog[type + id] || {};
                if (chatlog) {
                    layui.each(chatlog, function (index, item) {
                        if (Number(item.cid) === Number(cid)) {
                            item.isback = 1;
                            chatlog[index]=item;
                            $('.layim-chat-main').find('li.cid_'+cid).html($(laytpl(elemChatMain).render(item)).html());
                        }
                    });
                }
                local.chatlog[type + id] = chatlog;
                layui.data('layim', {
                    key: cache.mine.id
                    , value: local
                });
