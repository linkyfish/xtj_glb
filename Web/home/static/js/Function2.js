...
function GT_ShowADList(_strTagID, _strAreaID, _nCountType, _strATagExtString, _strIMGTagExtString,_strCallBackFunction) {
	...
	if (location.protocol == "https:")
		...
		strURL		= location.protocol + "//article.agent." + GetWebDomainName(location.hostname) + "/common/lib/Advertisement/GetADList.aspx" ;
...
function GT_ShowADs(_strTagID, _strAreaID, _nCountType, _strATagExtString, _strIMGTagExtString,_strCallBackFunction) {
	...
	if (location.protocol == "https:")
		...
		strURL		= location.protocol + "//article.agent." + GetWebDomainName(location.hostname) + "/common/lib/Advertisement/GetADs.aspx" ;
...
function GetArticleAD(JData) {
	...
	if (JData.AD_HTML != undefined) {
		...
		if (JData.TAG_ID.indexOf("MOBILE_INGAME") != -1)					// ex:ad_MOBILE_INGAME_ACTION_AD1
		{
			...
			if (AD_aLink.attr("href") != undefined)
				AD_aLink.attr("href", AD_aLink.attr("href") + "?NAI=" + escape(JData.Area_ID).substring(0, TrimStartIndex));	// ex:NAI=MOBILE_I371_ANDROID
